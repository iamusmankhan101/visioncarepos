...
Route::get('/check-voucher-tracking', function () {
    try {
        $voucherTrackingFile = storage_path('logs/voucher_tracking.log');
        $errorLogFile = storage_path('logs/php_errors.log');
        
        $result = [
            'voucher_tracking_exists' => file_exists($voucherTrackingFile),
            'error_log_exists' => file_exists($errorLogFile),
            'voucher_tracking_content' => file_exists($voucherTrackingFile) ? 
                array_slice(file($voucherTrackingFile, FILE_IGNORE_NEW_LINES), -10) : [],
            'error_log_content' => []
        ];
        
        // Get recent error log entries related to vouchers
        if (file_exists($errorLogFile)) {
            $lines = file($errorLogFile, FILE_IGNORE_NEW_LINES);
            $voucherLines = array_filter($lines, function($line) {
                return stripos($line, 'VOUCHER DETECTION') !== false;
            });
            $result['error_log_content'] = array_slice($voucherLines, -20);
        }
        ...
        if ($phpErrorLog && file_exists($phpErrorLog)) {
            ...
            $result['php_error_log_content'] = array_slice($voucherLines, -20);
        }
...
// Simple voucher test route
Route::get('/simple-voucher-test', function () {
    try {
        // Reset voucher for testing
        $voucher = \App\Voucher::where('code', '2')->first();
        if (!$voucher) {
        ...
        // Calculate expected discount for voucher "2" (10%)
        $expectedDiscount = ($subtotal * 10) / 100; // Should be 1.00
        ...
        $result = [
            ...
            'voucher_details' => [
                ...
                'used_count_before' => $voucher->used_count,
                'usage_limit' => $voucher->usage_limit
            ],
            'expected_discount' => $expectedDiscount,
            'actual_discount' => $discountAmount,
            'discount_matches' => abs($expectedDiscount - $discountAmount) < 0.01
        ];
        ...
        if (abs($expectedDiscount - $discountAmount) < 0.01) {
            ...
            $result['increment_test'] = [
                'used_count_after' => $voucher->used_count,
                'increment_worked' => $voucher->used_count > 0,
                'is_valid_after' => $voucher->isValid(100),
                'reached_limit' => $voucher->usage_limit && $voucher->used_count >= $voucher->usage_limit
            ];
        }
...
Route::get('/test-voucher-usage-tracking', function () {
    try {
        ...
        \Log::info('VOUCHER TEST: Laravel logging is working');
        error_log('VOUCHER TEST: PHP error_log is working');
        
        // Simulate a POS sale with discount
        $testData = [
            'subtotal' => 10.00,
            'discount' => 1.00, // 10% discount on 10.00
            'voucher_code' => '2',
        ...
        $matchingVouchers = \App\Voucher::where('business_id', $business_id)
            ->where('is_active', 1)
            ->where('code', $testData['voucher_code'])
            ->where(function($query) {
                $query->whereNull('expires_at')
                      ->orWhere('expires_at', '>', now());
            })
        ...
        $result = [
            'success' => true,
            'test_data' => $testData,
            'matching_vouchers_count' => $matchingVouchers->count(),
            'voucher_found' => $voucherFound ? true : false,
            'voucher_details' => $voucherFound ? [
                'code' => $voucherFound->code,
                'discount_type' => $voucherFound->discount_type,
                'discount_value' => $voucherFound->discount_value,
                'used_count' => $voucherFound->used_count,
                'usage_limit' => $voucherFound->usage_limit
            ] : null,
            'logging_test' => 'Check Laravel logs for test messages'
        ];
        ...
        if ($voucherFound) {
            ...
            \Log::info('VOUCHER TEST: Successfully incremented voucher', [
                'code' => $voucherFound->code,
                'before' => $beforeCount,
                'after' => $afterCount
            ]);
        ...
        \Log::error('VOUCHER TEST: Error occurred', ['error' => $e->getMessage()]);
        return response()->json(['error' => $e->getMessage()]);
    }
});