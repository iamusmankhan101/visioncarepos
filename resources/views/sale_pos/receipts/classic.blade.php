...
// Hide any browser-generated URLs
document.addEventListener('DOMContentLoaded', function() {
    // Remove any elements that might contain URLs
    var urlElements = document.querySelectorAll('[href*="digitrot"], [src*="digitrot"], *[class*="url"], *[id*="url"]');
    urlElements.forEach(function(element) {
        if (element.textContent && element.textContent.includes('digitrot.com')) {
            element.style.display = 'none';
        }
    });
    
    // Hide any text nodes containing the URL
    function hideURLText(node) {
        if (node.nodeType === Node.TEXT_NODE) {
            if (node.textContent && node.textContent.includes('digitrot.com')) {
                node.textContent = '';
            }
        } else {
            for (var i = 0; i < node.childNodes.length; i++) {
                hideURLText(node.childNodes[i]);
            }
        }
    }
    
    hideURLText(document.body);
});

// Also run before printing
window.addEventListener('beforeprint', function() {
    var urlElements = document.querySelectorAll('*');
    urlElements.forEach(function(element) {
        if (element.textContent && element.textContent.includes('digitrot.com')) {
            element.style.display = 'none';
        }
