...
class BusinessSelectionController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Full Business Setup - Create essential data if requested
            if ($request->has('full_business_setup') && $request->full_business_setup) {
                $this->createFullBusinessSetup($business, $user);
            }
    ...
    /**
     * Create full business setup with essential data
     */
    private function createFullBusinessSetup($business, $user)
    {
        try {
            // Create essential tax rates
            $taxRates = [
                ['name' => 'VAT@0%', 'amount' => 0],
                ['name' => 'VAT@5%', 'amount' => 5],
            ...
            Log::info('Full business setup completed for business: ' . $business->name);
            
        } catch (\Exception $e) {
            Log::error('Full business setup failed: ' . $e->getMessage());
            // Don't throw exception, just log it so business creation doesn't fail
        }
    }
}